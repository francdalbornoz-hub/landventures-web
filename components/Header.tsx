  ...
  return (
    ...
    >
      <div className="container-page flex items-center justify-between py-4 md:py-5">
        <Link href="/" aria-label={`${site.name} — Inicio`} className="block">
          <Image
            src="/images/logo.png"
            alt={`${site.name} logo`}
            width={2522}
        ...
        >
          ...
          <div className="w-6 h-5 relative flex flex-col justify-between">
            <span className={`block h-0.5 bg-white transition-transform ${open ? 'translate-y-2 rotate-45' : ''}`} />
      ...
      {open && (
        <nav className="lg:hidden border-t border-white/10 bg-ink" aria-label="Móvil">
          <ul className="container-page py-4 flex flex-col gap-1">
            {site.nav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block py-3 text-xs font-medium uppercase tracking-[0.2em] text-white hover:text-brand"
                >
                  {item.label}
                </Link>
              </li>
